...
open class FormNewStudentInviteFragment : FormFragment() {

    lateinit var invite: Invite
    override fun getLayoutResourceID() = R.layout.fragment_new_invite_student
    ...
    override fun blockFields(status: Boolean) {
        et_student_name.isEnabled = !status
        et_student_valor.isEnabled = !status
        et_student_expected_sales.isEnabled = !status
        bt_cancel_student.isEnabled = !status
        bt_nu_student.isEnabled = !status
    }
    ...
    fun getItens(){
        invite = Invite(1,(activity as MainActivity).user.party.id,"",1f,1,1f,1)
